...
- [x] `#25` DetailActivity — 이미지 원본 및 OCR 텍스트 표시 `UI`
- [x] `#26` 분류 카테고리 및 태그 표시 `UI`
- [x] `#28` 메모 작성 / 수정 로직 및 DB 연동 `DB` — DetailViewModel + SavedStateHandle (config change 대응)
...
- [x] `#37` Coroutine 전역 예외 처리 (CoroutineExceptionHandler) `안정성`
- [x] `#38` 대용량 이미지 OOM 방지 (BitmapFactory.Options) `안정성`
- [x] `#39` URI 권한 만료 대응 검증 `안정성`
- [x] `#36` 설정 화면 구현 `UI`
...
| Phase 5 — 상세 화면 & Drawer | 3 / 3 | 6 / 6 | 0 / 1 |
| Phase 6 — 안정성 & 마무리 | 4 / 4 | 2 / 4 | 0 / 1 |
| **합계** | **20 / 20** | **21 / 26** | **0 / 5** |
